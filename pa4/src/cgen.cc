...
void CgenClassTable::code_main()
{
	...
	string strPass("Main.main() returned %d\n");
	op_arr_type strType(INT8, strPass.length()+1);
	const_value strConst(strType, strPass, false);
...
void CgenNode::codeGenMainmain()
{
	...
	CgenEnvironment* newEnv = new CgenEnvironment(*(class_table->ct_stream), this);
	vector<operand> mainArgs;
	op_type i32_type(INT32);
	vp.define(*(class_table->ct_stream), i32_type, "Main_main", mainArgs);
